...
def abrir_analise_memorando():
    ...
    def executar_conciliacao():
        ...
        def _registrar_divergencia(tipo_divergencia: str, detalhes: str = ""):
            """
            Registra no Supabase (tabela public.log_divergencias) que a loja
            apresentou divergência no dia. Não bloqueia o fluxo em caso de falha.
            """
            try:
                loja_txt = entry_loja.get().strip()
                data_txt = entry_data.get().strip()

                # Fallback: tenta extrair do cabeçalho do PDF selecionado, se houver
                if (not loja_txt or not data_txt):
                    pdf_path_fb = entry_pdf.get().strip() if 'entry_pdf' in locals() or 'entry_pdf' in globals() else ""
                    if pdf_path_fb:
                        try:
                            loja_auto, data_auto = extrair_cabecalho_memorando(pdf_path_fb)
                            if not loja_txt and loja_auto:
                                loja_txt = loja_auto
                            if not data_txt and data_auto:
                                data_txt = data_auto
                ...
                if not loja_txt or not data_txt:
                    print("[WARN] Log de divergência não registrado: loja/data ausentes.")
                    return

                data_iso = _to_iso(data_txt)
                payload = {
                    "loja": str(loja_txt).strip(),
                    "data": data_iso,
                    "tipo_divergencia": tipo_divergencia,
                    "detalhes": detalhes,
                    "usuario": (current_user or {}).get("username") if 'current_user' in globals() else None,
                    "criado_em": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                }
                (
                    supabase
                    .schema("public")
                    .table("log_divergencias")
                    .insert(payload)
                    .execute()
                )
                print("[LOG DIVERGÊNCIA] Registrado:", payload)
            except Exception as e:
                # Não interrompe a conciliação; apenas avisa no console
                print("[WARN] Falha ao registrar log de divergência:", e)
        ...
        if abs(delta_dev_din) > 0.01 or abs(delta_dev_car) > 0.01:
            ...
            _registrar_divergencia(
                tipo_divergencia="Devolução à Vista",
                detalhes=f"Dinheiro: {_fmt(delta_dev_din)} | Cartão: {_fmt(delta_dev_car)}"
            )
        ...
        if tem_diferenca_prazo:
            ...
            _registrar_divergencia(
                tipo_divergencia="Devolução a Prazo",
                detalhes=mensagem_alerta.strip()
            )
